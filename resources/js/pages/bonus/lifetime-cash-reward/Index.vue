...
import { index as lifetimeIndex, destroy as lifetimeDestroy, release as lifetimeRelease, massRelease as lifetimeMassRelease } from '@/actions/App/Http/Controllers/BonusComission/BonusLifetimeCashRewardController';
...
const breadcrumbItems: BreadcrumbItem[] = [
    { title: 'Bonus Lifetime Cash Reward', href: lifetimeIndex.url() },
];
...
const handleDelete = () => {
    ...
    router.delete(lifetimeDestroy.url(deleteDialog.value.bonus.id), {
        preserveScroll: true,
...
const handleRelease = () => {
    ...
    releaseForm.post(lifetimeRelease.url(releaseDialog.value.bonus.id), {
        preserveScroll: true,
...
const handleMassRelease = () => {
    ...
    massReleaseForm.post(lifetimeMassRelease.url(), {
        preserveScroll: true,
...
const columns: ColumnDef<Bonus>[] = [
    ...
    },
    {
        accessorKey: 'status',
        header: () => 'Status',
        cell: ({ row }) => h(Badge, { variant: getStatusVariant(row.original.status), class: 'gap-1' }, () => [
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbItems">
        <Head title="Bonus Lifetime Cash Reward" />
        ...
        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            ...
            <div class="grid gap-4 md:grid-cols-4">
                <Card>
                    ...
                    <CardContent>
                        ...
                        <p class="text-xs text-muted-foreground">{{ statistics.count_pending }} reward</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle class="text-sm font-medium">Total Claimed</CardTitle>
                        <Receipt class="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                        <div class="text-2xl font-bold text-green-600">{{ formatCurrency(statistics.total_claimed) }}</div>
                        <p class="text-xs text-muted-foreground">{{ statistics.count_claimed }} reward</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle class="text-sm font-medium">Total Expired</CardTitle>
                        <XCircle class="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                        <div class="text-2xl font-bold text-red-600">{{ formatCurrency(statistics.total_expired) }}</div>
                        <p class="text-xs text-muted-foreground">{{ statistics.count_expired }} reward</p>
                    </CardContent>
                </Card>
            </div>
            ...
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div class="flex flex-1 items-center gap-4">
                    <div class="relative flex-1 md:max-w-sm">
                        ...
                        <Input v-model="search" placeholder="Cari member atau nama reward..." class="pl-9" />
                    </div>
                    <Select v-model="statusFilter">
                        ...
                        <SelectContent>
                            ...
                            <SelectItem value="1">Claimed</SelectItem>
            ...
            <!-- Pagination -->
            <Pagination :data="bonuses" :url="lifetimeIndex.url()" />
        </div>
        ...
        <ConfirmDialog :open="releaseDialog.open" title="Release Bonus" :description="`Apakah Anda yakin ingin merilis reward ${releaseDialog.bonus?.reward_name} sebesar ${releaseDialog.bonus ? formatCurrency(releaseDialog.bonus.reward) : ''} ke member?`" confirm-text="Release" @update:open="releaseDialog.open = $event" @confirm="handleRelease" />
