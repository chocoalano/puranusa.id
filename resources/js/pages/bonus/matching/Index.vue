...
import { index, create, show, destroy, release, massRelease } from '@/actions/App/Http/Controllers/BonusComission/BonusMatchingController';
...
interface Statistics {
    ...
    by_level: Record<number, { total: number; count: number }>;
}
        ...
        if (newSorting.length > 0) {
            router.get(
                index.url(),
                {
                    ...
                    level: levelFilter.value !== 'all' ? parseInt(levelFilter.value) : undefined,
    ...
    <AppLayout :breadcrumbs="breadcrumbItems">
        ...
        <div class="rounded-xl p-4 space-y-6 py-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold tracking-tight">Bonus Matching</h1>
                    <p class="text-muted-foreground">Kelola bonus matching multi-level</p>
                </div>
            ...
            <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                ...
                <Card class="md:col-span-2">
                    <CardHeader>
                        <CardTitle class="text-sm font-medium">By Level</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div class="grid grid-cols-5 gap-2 text-sm">
                            <div v-for="level in [1, 2, 3, 4, 5]" :key="level" class="text-center">
                                <div class="font-semibold text-xs text-muted-foreground">
                                    Level {{ level }}
                                </div>
                                <div class="font-bold">
                                    {{
                                        formatCurrency(
                                            (statistics.by_level && statistics.by_level[level]?.total) || 0
                                        )
                                    }}
                                </div>
                                <div class="text-xs text-muted-foreground">
                                    {{ (statistics.by_level && statistics.by_level[level]?.count) || 0 }} bonus
                                </div>
                            </div>
                        </div>
                    </CardContent>
