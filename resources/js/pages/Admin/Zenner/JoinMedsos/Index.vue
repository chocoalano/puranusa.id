...
import type { PaginatedData, ZennerFilters, JoinMedsos } from '@/types/zenner';

interface Props {
    items: PaginatedData<JoinMedsos>;
    ...
    <AppLayout>
        <div class="container mx-auto py-6 space-y-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold tracking-tight">Join Medsos</h1>
                    <p class="text-muted-foreground">Kelola media sosial Zenner Club</p>
                </div>
                <Button as-child>
                    <Link href="/admin/zenner/join-medsos/create">
                    ...
                    <div class="flex flex-col md:flex-row gap-4">
                        <div class="flex-1">
                            <div class="relative">
                                ...
                                <Input v-model="search" placeholder="Cari platform..." class="pl-10" @keyup.enter="applyFilters" />
                            </div>
                        </div>
                    ...
                    <Table v-if="items.data.length > 0">
                        ...
                        <TableBody>
                            <TableRow v-for="item in items.data" :key="item.id">
                                <TableCell>
                                    <p class="font-medium">{{ item.platform }}</p>
                                </TableCell>
                                <TableCell>
                                    <p class="text-sm text-muted-foreground truncate max-w-[200px]">{{ item.url }}</p>
                                </TableCell>
                                <TableCell>{{ item.followers_count.toLocaleString('id-ID') }}</TableCell>
                                <TableCell>
                                ...
                                <TableCell class="text-right">
                                    <div class="flex justify-end gap-2">
                                        <Button as-child variant="ghost" size="icon">
                                            <Link :href="`/admin/zenner/join-medsos/${item.id}/edit`">
        ...
        <AlertDialog :open="itemToDelete !== null" @update:open="itemToDelete = null">
            <AlertDialogContent>
                <AlertDialogHeader>
                    <AlertDialogTitle>Hapus Join Medsos?</AlertDialogTitle>
