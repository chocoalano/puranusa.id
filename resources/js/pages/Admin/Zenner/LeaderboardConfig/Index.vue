...
import type { PaginatedData, ZennerFilters, LeaderboardConfig } from '@/types/zenner';

interface Props {
    items: PaginatedData<LeaderboardConfig>;
...
const confirmDelete = () => {
    ...
    useForm({}).delete(`/admin/zenner/leaderboard-configs/${itemToDelete.value}`, {
        preserveScroll: true,
        onSuccess: () => { toast.success('Config berhasil dihapus'); itemToDelete.value = null; },
...
const goToPage = (page: number) => {
    router.get('/admin/zenner/leaderboard-configs', { page, search: search.value || undefined, status: status.value !== 'all' ? status.value : undefined, type: typeFilter.value !== 'all' ? typeFilter.value : undefined, per_page: perPage.value }, { preserveState: true, preserveScroll: true });
};
</script>
    ...
    <AppLayout>
        <div class="container mx-auto py-6 space-y-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold tracking-tight">Leaderboard Config</h1>
                    <p class="text-muted-foreground">Kelola konfigurasi leaderboard</p>
                </div>
                <Button as-child>
                    <Link href="/admin/zenner/leaderboard-configs/create"><Plus class="mr-2 h-4 w-4" />Tambah Config</Link>
                </Button>
            </div>
                    ...
                    <div class="flex flex-col md:flex-row gap-4">
                        <div class="flex-1">
                            <div class="relative">
                                ...
                                <Input v-model="search" placeholder="Cari config..." class="pl-10" @keyup.enter="applyFilters" />
                            </div>
                        </div>
                        <div class="flex gap-2">
                            <Select v-model="typeFilter" @update:model-value="applyFilters">
                                <SelectTrigger class="w-[160px]"><SelectValue placeholder="Tipe" /></SelectTrigger>
                                <SelectContent>
                    ...
                    <Table v-if="items.data.length > 0">
                        ...
                        <TableBody>
                            <TableRow v-for="item in items.data" :key="item.id">
                                ...
                                <TableCell>{{ item.entries_count || 0 }}</TableCell>
                                <TableCell>
                                ...
                                <TableCell class="text-right">
                                    <div class="flex justify-end gap-2">
                                        <Button as-child variant="ghost" size="icon"><Link :href="`/admin/zenner/leaderboard-configs/${item.id}`"><Eye class="h-4 w-4" /></Link></Button>
                                        <Button as-child variant="ghost" size="icon"><Link :href="`/admin/zenner/leaderboard-configs/${item.id}/edit`"><Pencil class="h-4 w-4" /></Link></Button>
        ...
        <AlertDialog :open="itemToDelete !== null" @update:open="itemToDelete = null">
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>Semua entries terkait juga akan dihapus. Lanjutkan?</AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
