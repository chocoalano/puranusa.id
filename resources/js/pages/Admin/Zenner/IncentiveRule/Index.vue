...
import type { PaginatedData, ZennerFilters, IncentiveRule } from '@/types/zenner';

interface Props {
    items: PaginatedData<IncentiveRule>;
...
const confirmDelete = () => {
    ...
    useForm({}).delete(`/admin/zenner/incentive-rules/${itemToDelete.value}`, {
        preserveScroll: true,
        onSuccess: () => { toast.success('Rule berhasil dihapus'); itemToDelete.value = null; },
...
const goToPage = (page: number) => {
    router.get('/admin/zenner/incentive-rules', { page, search: search.value || undefined, status: status.value !== 'all' ? status.value : undefined, type: typeFilter.value !== 'all' ? typeFilter.value : undefined, per_page: perPage.value }, { preserveState: true, preserveScroll: true });
};
</script>
    ...
    <AppLayout>
        <div class="container mx-auto py-6 space-y-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold tracking-tight">Skema Insentif</h1>
                    <p class="text-muted-foreground">Kelola rule insentif program Zenner Club</p>
                </div>
                <Button as-child>
                    <Link href="/admin/zenner/incentive-rules/create"><Plus class="mr-2 h-4 w-4" />Tambah Rule</Link>
                </Button>
            </div>
                    ...
                    <div class="flex flex-col md:flex-row gap-4">
                        <div class="flex-1">
                            <div class="relative">
                                ...
                                <Input v-model="search" placeholder="Cari rule..." class="pl-10" @keyup.enter="applyFilters" />
                            </div>
                        </div>
                        <div class="flex gap-2">
                            <Select v-model="typeFilter" @update:model-value="applyFilters">
                                <SelectTrigger class="w-[140px]"><SelectValue placeholder="Tipe" /></SelectTrigger>
                                <SelectContent>
                            ...
                            <TableRow v-for="item in items.data" :key="item.id">
                                ...
                                <TableCell class="text-right">
                                    <div class="flex justify-end gap-2">
                                        <Button as-child variant="ghost" size="icon"><Link :href="`/admin/zenner/incentive-rules/${item.id}/edit`"><Pencil class="h-4 w-4" /></Link></Button>
        ...
        <AlertDialog :open="itemToDelete !== null" @update:open="itemToDelete = null">
            <AlertDialogContent>
                <AlertDialogHeader>
                    <AlertDialogTitle>Hapus Rule Insentif?</AlertDialogTitle>
