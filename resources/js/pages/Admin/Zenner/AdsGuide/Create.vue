...
const submit = () => {
    form.post('/admin/zenner/ads-guides', {
        onSuccess: () => toast.success('Ads Guide berhasil dibuat'),
    });
};
</script>
        ...
        <Card>
            <CardHeader>
                <div class="flex items-center justify-between">
                    <h2 class="text-xl font-semibold">Tambah Ads Guide</h2>
                    <Button variant="outline" @click="router.visit('/admin/zenner/ads-guides')">Kembali</Button>
                </div>
            </CardHeader>
            <CardContent>
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div>
                        ...
                        <p v-if="form.errors.platform" class="mt-1 text-sm text-destructive">{{ form.errors.platform }}</p>
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-medium">Budget Range</label>
                        <Input v-model="form.budget_range" />
                        <p v-if="form.errors.budget_range" class="mt-1 text-sm text-destructive">{{ form.errors.budget_range }}</p>
                    </div>
