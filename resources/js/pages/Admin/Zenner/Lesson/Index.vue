...
import type { PaginatedData, ZennerFilters, Lesson, Course } from '@/types/zenner';

interface Props {
    items: PaginatedData<Lesson>;
...
const applyFilters = () => {
    router.get('/admin/zenner/lessons', {
        ...
        course_id: courseId.value !== 'all' ? courseId.value : undefined,
...
const confirmDelete = () => {
    ...
    useForm({}).delete(`/admin/zenner/lessons/${itemToDelete.value}`, {
        preserveScroll: true,
        onSuccess: () => { toast.success('Lesson berhasil dihapus'); itemToDelete.value = null; },
        ...
        <div class="container mx-auto py-6 space-y-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-muted-foreground">Kelola lesson Zenner Academy</p>
                </div>
                <Button as-child>
                    <Link href="/admin/zenner/lessons/create"><Plus class="mr-2 h-4 w-4" />Tambah Lesson</Link>
                </Button>
            </div>
                    ...
                    <div class="flex flex-col md:flex-row gap-4">
                        <div class="flex-1">
                            <div class="relative">
                                ...
                                <Input v-model="search" placeholder="Cari lesson..." class="pl-10" @keyup.enter="applyFilters" />
                            </div>
                        </div>
                        <div class="flex gap-2">
                            <Select v-model="courseId" @update:model-value="applyFilters">
                                <SelectTrigger class="w-[180px]"><SelectValue placeholder="Course" /></SelectTrigger>
                                <SelectContent>
                            ...
                            <Select v-model="perPage" @update:model-value="applyFilters">
                                <SelectTrigger class="w-[100px]"><SelectValue placeholder="Per halaman" /></SelectTrigger>
                                <SelectContent>
                                ...
                                <TableCell class="text-right">
                                    <div class="flex justify-end gap-2">
                                        <Button as-child variant="ghost" size="icon">
                                            <Link :href="`/admin/zenner/lessons/${item.id}/edit`"><Pencil class="h-4 w-4" /></Link>
                                        </Button>
